...
FRAME_INTRO_TEXT = (
    "다음은 영상을 프레임 단위로 분할한 것입니다. "
    "각 프레임의 정보를 참고하여 분석해주세요.\n\n"
)
FRAME_DEFAULT_DESC = "프레임의 시각 정보를 참고하세요."
FRAME_OUTRO_TEXT = "위 프레임들을 순서대로 분석하여 영상 속 상황을 설명해주세요."
...
def extract_video_frames(
    ...
    Returns:
        base64 인코딩된 이미지 data URI 리스트 (또는 메타데이터 리스트)
        return_dir=True이면 (리스트, 저장 디렉토리) 반환
    """
    ...
    for frame_idx in candidate_indices:
        ...
        frame_uri = f"data:image/jpeg;base64,{base64_data}"
    ...
    payload: List[Any] = frame_entries if return_metadata else frames
    if return_dir:
...
def build_interleaved_content_parts(
    frame_entries: List[Dict[str, Any]],
    user_prompt: str,
) -> Tuple[List[Dict[str, Any]], str]:
    content_parts: List[Dict[str, Any]] = []
    if not frame_entries:
        return content_parts, (user_prompt or "")

    content_parts.append({"type": "text", "text": FRAME_INTRO_TEXT})

    for idx, entry in enumerate(frame_entries, start=1):
        timestamp = entry.get("timestamp")
        if timestamp is None:
            label = f"[Frame {idx}] {FRAME_DEFAULT_DESC}\n"
        else:
            label = f"[Frame {idx} - {float(timestamp):.1f}s] {FRAME_DEFAULT_DESC}\n"

        content_parts.append({"type": "text", "text": label})
        content_parts.append({
            "type": "image_url",
            "image_url": {"url": entry.get("url", "")},
        })
    ...
    return content_parts, final_prompt_text
...
def analyze_video_file(file_path: str) -> str:
    ...
    if ext in ["jpg", "jpeg", "png", "gif", "webp"]:
        frame_entries.append({
            "url": encode_file_base64(file_path),
            "timestamp": None,
        })
        ...
        frame_entries.extend(frames)

    text_prompt = build_text_override(file_path) or ""
    if frame_entries:
        content_parts, _ = build_interleaved_content_parts(frame_entries, text_prompt)
    elif text_prompt:
...
def respond_multimodal(
    ...
    final_prompt_text = text or ""

    if frame_entries:
        content_parts, final_prompt_text = build_interleaved_content_parts(frame_entries, text or "")
    elif text:
        content_parts.append({"type": "text", "text": text})
    
    # Create message
    if len(content_parts) == 1 and content_parts[0].get("type") == "text" and not frame_entries:
    ...
    if video_paths and full_response:
        total_frames_used = sum(frames_used_counts) if frames_used_counts else None
    ...
    if frames_used_counts:
        frames_used_line = f"Frames used: {sum(frames_used_counts)}\n"
